/*
 * Snapshot scheduling and safe maintenance commands.
 *
 * Design goals:
 *   - deterministic named sessions
 *   - no duplicate snapshot for the same trading date/session/horizon
 *   - explicit trigger installation/removal
 *   - dry-run first maintenance
 *   - no destructive cleanup without an explicit confirmation token
 */
var SCHEDULER_AND_MAINTENANCE = (function () {
  'use strict';

  const VERSION = 'SCHED-MAINT-1.0.0';
  ...
  const SESSIONS = Object.freeze({
    SAME_DAY_EARLY: Object.freeze({
      functionName: 'scheduledSameDayEarlySnapshot_',
      hour: 10,
      minute: 5,
      horizon: 'SAME_DAY',
      sessionKind: 'SAME_DAY_EARLY'
    }),
    SAME_DAY_MID: Object.freeze({
      functionName: 'scheduledSameDayMidSnapshot_',
      hour: 12,
      minute: 30,
      horizon: 'SAME_DAY',
      sessionKind: 'SAME_DAY_MID'
    }),
    NEXT_DAY_CLOSE: Object.freeze({
      functionName: 'scheduledNextDayCloseSnapshot_',
      hour: 17,
      minute: 55,
      horizon: 'NEXT_DAY',
  ...
  function ensureRunLog_() {
    ...
    const current = sh.getRange(1, 1, 1, RUN_LOG_HEADERS.length).getValues()[0];
    const same = RUN_LOG_HEADERS.every(function (h, i) { return String(current[i] || '') === h; });
    if (!same) {
      if (sh.getLastRow() > 1) throw new Error('Run log şeması uyuşmuyor.');
  ...
  function isHoliday_(date, holidays) {
    const key = dateKey_(date);
    return (holidays || []).map(String).indexOf(key) >= 0;
  }
  ...
  function runSession(input) {
    ...
    if (!isTradingDay_(now, input.options || {})) {
      appendRun_({
        runId: runId, runTs: now.toISOString(), tradingDate: tradingDate,
        sessionKind: session.sessionKind, horizon: session.horizon,
        status: 'SKIPPED_NON_TRADING_DAY', rowCount: 0,
        message: 'İşlem günü değil.', durationMs: Date.now() - started
      });
    ...
    return acquireLock_(function () {
      if (hasSuccessfulRun_(tradingDate, session.sessionKind, session.horizon)) {
        appendRun_({
          ...
          message: 'Aynı işlem günü/session/horizon için başarılı snapshot mevcut.',
      ...
      try {
        if (typeof buildRuntimeInputs_ !== 'function') {
          throw new Error('buildRuntimeInputs_ adaptörü tanımlı değil.');
        }
  ...
  function installTriggers(options) {
    ...
    Object.keys(SESSIONS).forEach(function (key) {
      ...
      const trigger = ScriptApp.newTrigger(s.functionName)
        .timeBased()
        .atHour(s.hour)
        .nearMinute(s.minute)
        .everyDays(1)
        .inTimezone(TZ)
        .create();
      installed.push({ key: key, triggerId: trigger.getUniqueId(), functionName: s.functionName });
    });
    return installed;
  }
  ...
  function cleanupRunLog(input) {
    ...
    values.forEach(function (row, i) {
      const ts = new Date(row[1]).getTime();
      if (isFinite(ts) && ts < cutoff) removable.push(i + 2);
    });
  ...
  return Object.freeze({
    ...
    cleanupRunLog: cleanupRunLog,
    isTradingDay: isTradingDay_
  });
})();

function scheduledSameDayEarlySnapshot_() {
  return SCHEDULER_AND_MAINTENANCE.runSession({ session: SCHEDULER_AND_MAINTENANCE.sessions.SAME_DAY_EARLY });
}
function scheduledSameDayMidSnapshot_() {
  return SCHEDULER_AND_MAINTENANCE.runSession({ session: SCHEDULER_AND_MAINTENANCE.sessions.SAME_DAY_MID });
}
function scheduledNextDayCloseSnapshot_() {
  return SCHEDULER_AND_MAINTENANCE.runSession({ session: SCHEDULER_AND_MAINTENANCE.sessions.NEXT_DAY_CLOSE });
}
