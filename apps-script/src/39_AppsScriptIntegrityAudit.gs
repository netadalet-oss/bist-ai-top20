...
var APPS_SCRIPT_INTEGRITY_AUDIT = (function () {
  ...
  const VERSION = 'APPS-SCRIPT-INTEGRITY-1.1.0';
  ...
  const REQUIRED_GLOBALS = Object.freeze([
    ...
    { name:'LEGACY_FUNCTION_MIGRATION_AUDIT', kind:'object', layer:'migration' }
  ]);
  ...
  const REQUIRED_FUNCTIONS = Object.freeze([
    ...
    { name:'auditLegacyFunctionMigration_', layer:'migration' },
  ...
  function globalObject_() { return typeof globalThis !== 'undefined' ? globalThis : this; }
  ...
  function inspectSymbols_(root, specs) {
    return specs.map(function (spec) {
      ...
      return { name:spec.name, layer:spec.layer, expectedType:spec.kind || 'function',
        actualType:exists ? typeof root[spec.name] : 'undefined', exists:exists,
  ...
  function inspectMethods_(root) {
    const contracts = [
      ...
      { owner:'LEGACY_FUNCTION_MIGRATION_AUDIT', methods:['audit','assertValid'] }
    ];
    const rows = [];
    contracts.forEach(function (contract) {
      const owner = root[contract.owner];
      contract.methods.forEach(function (method) {
        rows.push({ owner:contract.owner, method:method, valid:!!owner && typeof owner[method] === 'function' });
      });
    });
    return rows;
  }
  function inspectRuntimeChain_(root) {
    const chain = [
      'LEGACY_FUNCTION_MIGRATION_AUDIT.assertValid',
    ...
    function resolve_(path) {
      const parts = path.split('.'); let value = root;
      for (let i=0;i<parts.length;i++) { if (value == null) return undefined; value = value[parts[i]]; }
      return value;
    }
    return chain.map(function (path,index) { return { order:index+1, callable:path, valid:typeof resolve_(path) === 'function' }; });
  }
  ...
  function audit(options) {
    ...
    const migration = root.LEGACY_FUNCTION_MIGRATION_AUDIT && typeof root.LEGACY_FUNCTION_MIGRATION_AUDIT.audit === 'function'
      ? root.LEGACY_FUNCTION_MIGRATION_AUDIT.audit({ root:root })
      : null;
    const legacyCollisions = migration ? migration.detectedLegacyCollisions || [] : [];
    ...
    return {
      ...
      valid:missing.length === 0 && missingMethods.length === 0 && brokenChain.length === 0 &&
        !!migration && migration.valid === true,
      notes:[
        ...
        'The V_141225 monolith must not be deployed together with modular files.',
  ...
  function assertValid(options) {
    ...
    if (!report.valid) {
      ...
      if (report.legacyCollisions.length) parts.push('legacy=' + report.legacyCollisions.join(','));
  ...
  return Object.freeze({ version:VERSION, requiredGlobals:REQUIRED_GLOBALS,
    requiredFunctions:REQUIRED_FUNCTIONS, audit:audit, assertValid:assertValid });
})();
...
function assertAppsScriptIntegrity_() { return APPS_SCRIPT_INTEGRITY_AUDIT.assertValid(); }
